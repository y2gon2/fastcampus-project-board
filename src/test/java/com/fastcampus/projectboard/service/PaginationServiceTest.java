...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.NONE, classes = PaginationService.class) // Test 경량화 option
class PaginationServiceTest {
    ...
    @MethodSource        // 동일한 method 명의 static 함수를 Given 함수로 사용할 수 있게 됨??
    @ParameterizedTest(name = "[{index}] 현재 페이지: {0}, 총 페이지: {1} => {2}")   // 값을 연속적으로 주입해서 동일한 method 의 동작을 여러번 확인할 수 있음 ??
    ...
    static Stream<Arguments> givenCurrentPageNumberAndTotalPages_whenCalculating_thenReturnsPaginationBarNumbers() {
        return Stream.of(
                // 내부에 검증하고 싶은 값들을 쭉 나열하여 하나씩 test 진행. -> @ParameterizedTest
                // Page 번호는 내부적으로 0번 부터 시작.
                // 총 123개 글이므로  13개 페이지 존재
    ...
    @DisplayName("현재 설정되어 있는 pagination bar 의 길이를 알려준다. (설정 (spec) 을 확인하는 작업)")
    @Test
