...
class UserAccountServiceTest {
    ...
    void givenExistentUserId_whenSearching_thenReturnsOptionalUserData() {
        ...
        given(userAccountRepository.findById(username))
                .willReturn(Optional.of(createUserAccountWithOnlyUsrname(username)));
    ...
    @DisplayName("회원정보를 입력하면, 새로운 회원 정보를 저장하여 가입시키고 해당 회원 데이터를 반환한다.")
    @Test
    void givenUserParams_whenSaving_thenSaveUserAccount() {
        // Given
        UserAccount userAccount = createUserAccountWithOnlyUsrname("uno");
    ...
    private UserAccount createUserAccountWithOnlyUsrname(String username) {
